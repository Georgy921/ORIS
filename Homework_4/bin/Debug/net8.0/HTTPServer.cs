...
public class HTTPServer
{
    ...
    public void SendStaticResponse(HttpListenerContext context, HttpStatusCode statusCode, string path)
    {
        ...
        if (response.StatusCode == 200)
            Global.Logger.LogMess($"Загружен {request.Url.AbsolutePath} {request.HttpMethod} - Status: {response.StatusCode}");
        else
            Global.Logger.LogMess($"Ошибка запроса: {request.Url.AbsolutePath} {request.HttpMethod} - Status: {response.StatusCode}");
    }
    ...
    public void SendJsonResponse(HttpListenerContext context, HttpStatusCode statusCode, string jsonString = "")
    {
        ...
        if (response.StatusCode == 200)
            Global.Logger.LogMess($"Запрос обработан: {request.Url.AbsolutePath} {request.HttpMethod} - Status: {response.StatusCode}");
        else
            Global.Logger.LogMess($"Ошибка запроса: {request.Url.AbsolutePath} {request.HttpMethod} - Status: {response.StatusCode}");
    }
    ...
    public void Send404Response(HttpListenerContext context, string path)
    {
        ...
        finally
        {
            if (context.Response.StatusCode == 200)
                Global.Logger.LogMess($"Запрос обработан: {context.Request.Url.AbsolutePath} {context.Request.HttpMethod} - Status: {context.Response.StatusCode}");
            else
                Global.Logger.LogMess($"Ошибка запроса: {context.Request.Url.AbsolutePath} {context.Request.HttpMethod} - Status: {context.Response.StatusCode}");
